...
pub async fn bootstrap() -> anyhow::Result<i32> {
    let Some(boot) = Bootstrap::get_from_env()? else {
        anyhow::bail!(
            "bootstrap: no bootstrap mode configured (HYPERACTOR_MESH_BOOTSTRAP_MODE unset)"
        );
    };
...
mod tests {
    ...
    fn test_bootstrap_mode_env_string_none_config_proc() {
        ...
        match round {
            Bootstrap::Proc { config: None, .. } => {}
            other => panic!("expected Proc with None config, got {:?}", other),
        }
    }
